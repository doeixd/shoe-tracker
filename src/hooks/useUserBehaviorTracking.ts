...
export function useUserBehaviorTracking() {
  ...
  const [behaviorPatterns, setBehaviorPatterns] =
    useState<BehaviorPatterns | null>(null);
  ...
  useEffect(() => {
    ...
    if (
      sessionData.currentRoute !== currentPath &&
      sessionData.routeHistory.length > 0
    ) {
    ...
    const analyzeAndUpdate = async () => {
      try {
        ...
        setBehaviorPatterns({
          ...
          averageSessionDuration: 0, // analysis.patterns.averageRouteTime || 0,
          preferredInteractionType: getMostCommonInteractionType(
            sessionData.interactions,
          ),
          timeOfDayPreferences: getTimeOfDayPreferences(
            sessionData.routeHistory,
          ),
  ...
  const getRouteRecommendations = useCallback(
    (route: string) => {
  ...
  const getSessionSummary = useCallback(() => {
    return {
      ...
      averageRouteTime:
        sessionData.routeHistory.length > 0
          ? sessionData.routeHistory.reduce(
              (sum, route) => sum + route.duration,
              0,
            ) / sessionData.routeHistory.length
          : 0,
...
export function useRouteBehaviorTracking(route: string) {
  const { trackInteraction, getRouteRecommendations, behaviorPatterns } =
...
function getMostCommonInteractionType(interactions: UserInteraction[]): string {
  ...
  return (
    Object.entries(counts).sort(([, a], [, b]) => b - a)[0]?.[0] || "click"
  );
}
